...
def generate_traj_txt(c2ws_anchor,H,W,fs,c,phi, theta, r,frame,device,viz_traj=False, save_dir = None):
    ...
    if len(phi) == 0:
        raise ValueError("Phi parameter cannot be empty. Please provide at least one phi value.")
    if len(theta) == 0:
        raise ValueError("Theta parameter cannot be empty. Please provide at least one theta value.")
    if len(r) == 0:
        raise ValueError("R parameter cannot be empty. Please provide at least one r value.")
...
def txt_interpolation(input_list,n,mode = 'smooth'):
    # Add input validation
    if len(input_list) == 0:
        raise ValueError("Input list cannot be empty for interpolation")

    # Handle single point case: return repeated value
    if len(input_list) == 1:
        return np.full(n, input_list[0])
...
def visualizer_frame(camera_poses, highlight_index):
    ...
    # 在Matplotlib 3.7.x中使用tostring_rgb()方法
