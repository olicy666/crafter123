...
try:
    import xformers
    import xformers.ops
    XFORMERS_IS_AVAILBLE = False  # 禁用xformers
except:
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, geo_bias=None):
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, **kwargs):
        ...
        # Handle geo_bias if provided
        geo_bias = kwargs.get('geo_bias', None)

        if mask is not None:
            forward_mask = partial(self._forward, mask=mask, geo_bias=geo_bias)
            return checkpoint(forward_mask, (x,), self.parameters(), self.checkpoint)

        # If no mask, pass geo_bias to _forward
        forward_func = partial(self._forward, geo_bias=geo_bias)
        return checkpoint(forward_func, input_tuple, self.parameters(), self.checkpoint)


    def _forward(self, x, context=None, mask=None, geo_bias=None):
        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None, mask=mask, geo_bias=geo_bias) + x
        x = self.attn2(self.norm2(x), context=context, mask=mask, geo_bias=geo_bias) + x
